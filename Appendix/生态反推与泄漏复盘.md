...
报告称：fork 把 `events.ts` 里的 `callback.bind(thisArg)` 改成 `Reflect.apply`，以减少闭包分配、降低 GC 压力。
...
- 上游标准 cordis 的 **5 个现役派发方法确实全部用 `Reflect.apply`**：`parallel`(events.ts:91)、`emit`(:98)、`serial`(:104)、`bail`(:111)、`waterfall`(:122)`[verified]`；
...
| §7.2 `events.ts` 改 `Reflect.apply` | `[verified]`（更正） | **对上游成立**：标准 cordis 5 个派发方法用 `Reflect.apply`（events.ts:91/98/104/111/122）；dsh vendored 的 rc.7 更早、仍 `.bind`，属版本差异 |
