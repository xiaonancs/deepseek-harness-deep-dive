...
社区普遍把 `dsh` 与 Claude Code、Codex CLI、Pi、Cascade 归为同类——都是跑在终端/网页里的 agent harness`[claimed]`（《全网调研》E.1）。被社区点名夸的差异点有两处：工具调用采用"严格的 JSON schema"（即对工具的输入输出格式有明确、强校验的定义），以及 Cordis 论文带来的一股"研究味"`[claimed]`（《全网调研》E.2-3）。
...
- **不收外部 PR**：CONTRIBUTING 明说"早期阶段、暂不接受外部 PR"，转而鼓励大家做插件、给仓库打上 `dsh-plugin` topic 标签、写教程`[verified]`（CONTRIBUTING.md:9-18）。这看似矛盾（拒代码却推生态），下面的 ratify-note 会分析它其实自洽。
