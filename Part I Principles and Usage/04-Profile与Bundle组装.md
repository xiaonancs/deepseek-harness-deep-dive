...
前一章的 Spatiotemporal Composability 是范式层的保证；到了产品层，真正要回答的痛点很具体：**同一个 `dsh` 二进制，如何既能起一个带浏览器界面的长驻应用，又能起一个跑完一个任务就退出的无头 runner，还允许用户在不改任何源码的前提下换掉沙箱后端、关掉遥测、加装一个自建插件？**
