...
之所以直接复用 Cordis 原生事件、而不是另起一套 koa-compose 式中间件栈或显式"阶段状态机"，根子上是因为整个 harness 已经建在 Cordis 底座上：listener 本身就是 Cordis effect，天然带热重载与卸载回滚，再造一套 dispatch / disposal / reload 语义等于在框架之上重复造框架，还得自己保证卸载正确性（`microkernel-event-taxonomy.md:24` `[verified]`）。
