...
> 本章讲 `dsh` 如何把"一个 agent 的整段交互史"表达为一条 **append-only 的类型化事件流**（`SessionEvent`），并从这条流投影出模型看到的消息历史。读完你能回答：为什么这里用事件溯源而不是直接存一个消息数组？"模型可见 ⟺ 已记录"这条不变量在运行时是怎么被断言的？`deriveMessages` 如何从日志重建请求？fork / resume / transcript / telemetry（遥测，即把运行时数据采集出来供观测分析）/ persistence 为什么全都能从同一条流派生？以及版本号为什么停在 `0`。
...
这条日志的形态由两个类型定义：`SessionEventMap` 是事件词表（每个 key 是一种事件类型、value 是其 payload），`SessionEvent<T>` 是日志里的一条条目（`packages/core/session/src/types.ts:236` 与 `:404`）。一条事件除判别标签 `type` 外，还带三个固定字段——`seq`（单调序号，恒等于 `log.length`）、`time`（epoch 毫秒）、`data`（payload）`[verified]`（`types.ts:405-411`）。
...
- 原始 `assistant/chunk` 是 replay/UI（User Interface，用户界面）数据，在派生中被跳过（组装好的 message 才权威）；`turn/*`、`step/*`、log-only 事件都投影为 null `[verified]`（`surface.ts:109-113`）。
...
顺带把版本号说清：`SESSION_FORMAT_VERSION` 一直 pin 在 `0`、不作任何兼容承诺，这是发布前姿态（`AGENTS.md` "Pre-release stance"）。它是个单调整数，且 version bump 由**写方**决定——只有结构性改动（header 形状、事件信封、核心事件语义、surface 机制）才 bump，普通新增事件类型交给 `ignorable` 覆盖、不 bump；换言之"parses without error"不是及格线，只有当老 runtime 无法再语义正确地处理新日志时才升版本（`types.ts:33-56`、`2026-08-10` Decision `[verified]`）。
...
- **`session/end-seed` 边界易被误写**：这个 log-only 事件标记构造 seed 结束，是 `firstLiveSeq` 的持久化投影；只有 `Session` 构造函数是合法写者，插件写它会把它之前的所有 live bracket（"活的成对区段"，即一对"开始…结束"标记之间的那段，如 `compaction/start` 到 `compaction/end`）静默归类为 seed 历史 `[verified]`（`types.ts:310-333`、`index.ts:545-547`）。读时要找**最后一个** `session/end-seed`（seed 已以它结尾就不重标，所以未改动地重开会话不会每次增长日志）。
...
这一点在 fork / resume 上体现得最直接：`dsh` 的 `fork(source, boundary?, childId?)` 只是取源日志一个"不在开放回合内"的稳定前缀（结束在开放回合内会直接抛 `OPEN_TURN` 拒绝）deep-clone 成子会话 seed，附上 `parentSession`/`seedLength`/继承 `cwd` 元数据，子会话的 messages / header / transcript 全部从 seed 自然重投影出来（`index.ts:1081-1138` `[verified]`）——因为唯一状态就是日志，fork 不可能漏拷某个隐藏状态。而竞品常见的"分叉时深拷贝一份内部对话状态/缓存"，则要精确枚举并拷贝所有内部状态，漏一个就得到与父不一致的子会话。resume / replay 同理是这条流的重投影（`ctx.sessions.create(id,{seed})` 是底层原语），crash recovery 则用 `interruptedTurnClosers` 合成缺失的 tool/step/turn 边界，补出一个 provider-valid 的 transcript（`repair.ts:27` `[verified]`）。
...
- docs/ADR（Architecture Decision Record，架构决策记录）：`docs/subsystems/session.md`；`.agents/notes/implemented/architecture/2026-06-11-event-sourced-sessions.md`；`.agents/notes/implemented/architecture/2026-08-10-session-log-version-mechanism.md`
