...
先纠正一个几乎所有人第一眼都会犯的误读：**seam（接缝）不是那个 interface**。`dsh` 的官方词表把它定义成一个"可替换能力"的三个角色的总和——一个 **Service Definition**（拥有 `ctx.<key>` 与词汇类型的 Cordis `Service`）、一个或多个 **Service Provider**（实现/注册契约的插件）、一个或多个 **Consumer**（注入服务键、面向模型或其他插件编程的一方）。`packages/shell` 是范本：`dsh-shell` 是 Definition、`dsh-bash-local`/`dsh-bash-sandbox` 是 Provider、`dsh-tool-bash` 是 Consumer。词表明确写道："seam 是完整能力，绝不是单个角色；单独一个角色不构成接缝"（docs/glossary.md:9 `[verified]`）。

为什么要把"契约/实现/消费面"这三件事分开命名、甚至分开打包？因为它们**以不同的速率、为不同的原因变化**。Agent Note 把这层动机写得很直白：契约是"这个能力是什么"、实现是"它怎么跑"、Consumer API 是"模型和其他插件对着什么编程"；把三者塞进一个包会把这三条变化速率耦合起来——"把本地执行器换成沙箱执行器，本不该动模型看到的工具 schema，却会连带搅动它"（`.agents/notes/implemented/architecture/2026-06-13-capability-seams.md:9 [verified]`）。
...
一个容易混淆的点：Cordis 本身已经用 services + `inject` 回答了"谁在运行时提供、谁需要"——provider 注册 `ctx.shell`，consumer 声明 `inject: ['shell']`，其 fiber 会 pend 到服务出现为止。那还要三角色干嘛？Agent Note 的回答很关键：**inject 机制是必要的，但它不决定包边界；这份 Agent Note 才决定**（capability-seams.md:11 `[verified]`）。

换句话说，`inject` 解决的是"时序依赖"（空间可组合性，见第 3 章），而三角色解决的是"变化边界如何切包"。两者正交。痛点在于：一个 harness 的能力天生是要换的——今天本地 bash，明天沙箱、远程执行器、另一家模型。如果不在包层面把"契约"和"实现"劈开，每次换后端都要冒着改动模型面契约的风险，而模型面契约一动，工具 schema、prompt、快照全跟着抖。
...
- `dsh-tool-bash`（Consumer）的运行时 `dependencies` 只有 `schemastery`；它对 `dsh-shell`（Definition）等接口的依赖挂在 **`peerDependencies`**（运行时契约依赖），而具体 Provider `dsh-bash-local` 只在 `devDependencies`（仅测试装配用）——**这条 peerDependency 正是"Consumer 依赖 Definition、不依赖 Provider"在包边界上的编码**（tool-bash/package.json `[verified]`）。
...
于是依赖箭头全部指向中间的 Definition，Provider 与 Consumer 之间**没有编译期连线**。这就是"一次替换改变整个产品"的机械基础：换 Provider 只改注册进 `ctx.shell` 的那个类，Consumer 的工具 schema 一个字都不用动。Agent Note 用一句话收口："沙箱执行器替换 `dsh-bash-local`，不碰任何工具 schema"（capability-seams.md:24 `[verified]`）。
...
三角色最有威力的推论，是**跨接缝的 provider 协同**。`dsh` 的架构文档给了一句纲领："文件系统与 subprocess 的 provider 共享同一个执行世界，把它们指向远程沙箱，Bash、PTY、LSP 会随之一起搬家，不需要任何 provider 分叉"（docs/architecture.md:102 `[verified]`）。

这句话的机械原理，藏在"谁 inject 了 `ctx.subprocess`"里。subprocess 是一个独立接缝（Service Definition = `dsh-subprocess`，Provider = `subprocess-local`/`subprocess-e2b`）。而它的 Consumer 是一大票别的接缝的 **Provider**：
...
于是只要把 `ctx.subprocess` 从 `subprocess-local` 换成 `subprocess-e2b`，这三条 Provider 全都在新的执行世界里起进程。而 `ctx.e2b` 服务"持有一个共享 E2B SDK 句柄与远程工作目录，让两个 E2B provider（fs-e2b、subprocess-e2b）栖身同一个 Linux 运行时"（docs/capability-seams.md:446 `[verified]`）。fs 换成 fs-e2b、subprocess 换成 subprocess-e2b，文件读写与进程执行就落在同一台远程机器上——**Bash/PTY/LSP 一次搬完**。
...
如果说 shell 展示了"接缝怎么切"，subagent 展示了**一个接缝能容纳多大的实现跨度**。它与 bash 的一个关键区别写在 Definition 的模块注释里："不像 bash（一个 context 一个执行器，第二次加载即抛错），subagent 是**命名 provider 注册表**，多个 provider 共存、调用方按名字挑一个，形状仿照 LLM 适配器注册表"（subagent/subagent/src/index.ts:6-10 `[verified]`）。
...
从"进程内一个新子 agent"到"委派给 OpenAI Codex / Claude Code 这样的另一个产品"，跨度全被同一个 `ctx.subagents` 契约吸收——而模型面的 `tool-subagent` 只对着这个契约编程，它不知道也不需要知道背后跑的是自家 spawn 还是别家 CLI。
...
社区对这套 DI 的实用性有真实分歧。全网调研记录了两种口径：一派赞"强制 cleanup 模式好"，一派质疑"跨插件 DI 的实际收益存疑"（调研 D 节 `[claimed]`）。这里必须对抗式处理。
...
横向看，Claude Code、Codex CLI 等同类 harness 更多以"配置 + 内建后端"提供有限替换；`dsh` 把替换点提升为**架构级的命名接缝**，代价是包数量（219 个 `@deepseek-ai/dsh-*` workspace 包）与样板，收益是第三方可在不改核心的前提下加 provider。这与其"开源即贡献、护城河是团队与文化"的理念一致（此为动机层判断，`[inferred]`）。
...
**一个命名陷阱被专门点名。** Agent Note 警告：不要把能力接缝的 `capability` 与 `@cordisjs/plugin-capability` 混淆——后者是**权限/安全**服务（命名权限 + 继承 + `ctx.capability.test`），是完全不同的轴，属于 deferred 的 permissions/sandbox 工作，"混淆二者正是这份 Agent Note 要命名的陷阱"（capability-seams.md:34 `[verified]`）。
