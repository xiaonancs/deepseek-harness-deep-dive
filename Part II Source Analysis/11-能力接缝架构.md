...
于是依赖箭头全部指向中间的 Definition，Provider 与 Consumer 之间**没有编译期连线**——它们俩谁也不认识谁，只各自认识中间那份契约。这就像插座标准：台灯和发电厂之间没有一根直连的电线，都只接到墙上那个统一的插孔。这也正是"一次替换改变整个产品"的机械基础：换 Provider 只改注册进 `ctx.shell` 的那个类，Consumer 的工具 schema 一个字都不用动。Agent Note 用一句话收口："沙箱执行器替换 `dsh-bash-local`，不碰任何工具 schema"（capability-seams.md:23 `[verified]`）。

> **↔ 论文对应**：Definition/Provider/Consumer 三角色与 `inject`/`provide`，在《Spatiotemporal Composability》论文里被形式化为**响应式 coeffect**——组件把依赖声明为 specification，在 coeffect context $\Sigma$ 上 provide/get，context 每次变化经 $\mathrm{notify}$ 按激活/失活/中性三态分类（见 [Part IV 论文全解](../Part%20IV%20Foundational%20Paper/22-A-Programming-Paradigm-for-Spatiotemporal-Composability.md) §3.2，Def.22/25/26）。"换 provider 改变整个产品"对应论文的**空间可组合性**：依赖满足才转移、provider 存活严格包住 consumer（Thm.63 Ordering），且一次转移不跨两次 coeffect 解析（Thm.64 Resolution coherence）`[verified]`。

## 四、实现细节关键点：共享一个执行世界

三角色最有威力的推论，是**跨接缝的 provider 协同**——多处接缝共用同一个"底座能力"，于是换掉这一个底座，上面一排能力会一起搬家。`dsh` 的架构文档给了一句纲领："文件系统与 subprocess 的 provider 共享同一个执行世界，把它们指向远程沙箱，Bash、PTY、LSP 会随之一起搬家，不需要任何 provider 分叉"（docs/architecture.md:102 `[verified]`）。这里的 subprocess 指"起子进程"这件底层能力，PTY（pseudo-terminal，伪终端，即操作系统模拟出的一块终端，用来跑那些需要交互的命令行程序）指伪终端那层，LSP（Language Server Protocol，语言服务器协议，即编辑器与"语言服务器"之间用来问补全、跳转的一套标准）指给代码补全/跳转的后台服务。
...
于是只要把 `ctx.subprocess` 从 `subprocess-local` 换成 `subprocess-e2b`，这三条 Provider 全都在新的执行世界里起进程——它们各自的代码没动一行，只是"起进程"这件事被换到了别处发生。而 `ctx.e2b` 服务（E2B 是一个跑远程沙箱的云服务；SDK 即 Software Development Kit，软件开发工具包，厂商封装好、拿来即用的一套库）"持有一个共享 E2B SDK 句柄与远程工作目录，让两个 E2B provider（fs-e2b、subprocess-e2b）栖身同一个 Linux 运行时"（docs/capability-seams.md:446 `[verified]`）。fs 换成 fs-e2b、subprocess 换成 subprocess-e2b，文件读写与进程执行就落在同一台远程机器上——**Bash/PTY/LSP 一次搬完**。这就像把家里的总闸从"市电"切到"备用发电机"：屋里所有插着的电器不用动，全体一起改喝新电源。
...
- `subagent-claude-code`：每次委派调用官方 Agent SDK，把 SDK 生出的真实 CLI（Command-Line Interface，命令行界面，即在终端里敲命令来驱动的那种程序）挂到共享 subprocess owner 下。
...
社区对这套 DI（Dependency Injection，依赖注入，即"我需要什么由框架递给我，而不是自己去 new"的那套做法）的实用性有真实分歧。全网调研记录了两种口径：一派赞"强制 cleanup 模式好"（框架逼着你写好资源清理，不容易漏），一派质疑"跨插件 DI 的实际收益存疑"（觉得为了换后端搞这么多间接层不划算）。（调研 D 节 `[claimed]`）这里必须对抗式处理——把两种说法都摆上台面认真掂量，而不是先站队。掂量下来，收益是真实的，但要**限定语境**：它并非来自"DI 本身"，而来自"能力天生多 provider、且 provider 间共享执行世界"这一事实——此时那层依赖注入的间接恰好落在真正的变化点上（换一处 `subprocess` 即迁移 Bash/PTY/LSP，收益在"扇出"处兑现）。反过来，对只有单个 provider 的能力（如 `ctx.webServer` 等标为 `core` 而非 `seam` 的服务），dsh 自己也没上三角色。（源码扇出 `[verified]`，社区分歧 `[claimed]`）
...
**样板成本是显性代价。** Agent Note 的 Consequences 一节自陈：分角色"增加了包和样板——`package.json`、`tsconfig`、README、注入接线"（capability-seams.md:38 `[verified]`）。这是三角色的直接税。

**何时拆、何时折，是一条需要判断力的边界。** dsh 给的规则是"角色独立演化才拆，别抢先拆，第二个 provider 出现前保持单包"（capability-seams.md:25 `[verified]`）——把判断权交给"变化速率"这个第一性标准，而非教条：LLM 接缝就据此把 Definition 与 Consumer 折进 `dsh-llm`（Consumer 就是 loop 本身，不是可换的 schema 面）、adapter 单列为 Provider 包，而 bash 则三包全开。这条规则的代价是"是否独立演化"要靠预判未来变化，可能拆错；好在 dsh 的 pre-release stance（AGENTS.md "foundation over blast radius"）允许自由重命名、重打包来纠偏，摊薄了这份风险。
