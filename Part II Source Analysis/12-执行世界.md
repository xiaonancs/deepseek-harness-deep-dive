...
先厘清一处容易被当成缺陷的边界：持久 PTY 的能力范围是**记录在案的有意收敛**，而非遗漏。persistent-pty Agent Note 白纸黑字声明只支持交互 shell 与行式 REPL（Linux/macOS），而全屏终端应用、按键序列、BEL 触发的控制流、进程丢失后的会话恢复、跨 agent 会话共享都明确延迟——遇到全屏 TUI（如 `htop`、`vim` 全屏模式）会碰壁，但那是范围声明，`bash/read/write/edit` 仍是可靠默认（`.agents/notes/implemented/feature/2026-07-16-persistent-pty-sessions.md` `[verified]`）。
