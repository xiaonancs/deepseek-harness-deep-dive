...
> 你有没有想过，当 AI 助手替你敲下一行命令时，背后到底发生了什么？本章讲 dsh 如何把"让模型跑命令"这件看似简单的事，拆成三层能力接缝（capability seam，可理解为一道"标准插槽"：接口固定，插头可整块替换）：`ctx.subprocess`（本地进程树）→ `ctx.shell`（bash/pwsh 执行器）→ `ctx.terminals`（持久 PTY（pseudo-terminal，伪终端，即操作系统模拟出来、供交互式命令行程序使用的终端）会话）。读完你能回答三个问题：为什么一次 `bash` 工具调用要穿过三个包；`resolve(request): Spec` 这个"显式默认化"步骤到底解决了什么；持久终端会话凭什么能跨工具调用、跨插件重载存活。
...
- **`ctx.terminals`** —— 持久 PTY 会话注册表。它给每个 Agent 一个能跨工具调用保留 cwd（current working directory，当前工作目录）/环境/后台任务的交互式终端，会话所有权绑定到精确的 `Agent`。`[verified]`（`packages/terminal/terminal/src/index.ts:105`）
...
4. **状态语义分层**：`ls` 这种一次性命令、`npm run dev &` 这种后台长任务、`python` REPL（Read-Eval-Print Loop，读取-求值-打印循环，即敲一行、立刻出结果的交互式解释器）这种需要跨调用保留状态的交互会话，是三种根本不同的生命周期。
5. **可替换**：本地进程、E2B（一个提供远程沙箱的云服务，可把命令跑在一台远端 Linux 机器上）远程沙箱、Windows pwsh——底层执行世界要能整包换掉而不惊动上层工具，就像换了台服务器却不用重写业务代码。
...
**pwsh 是 bash 的逐调用镜像。** `pwsh-local`（Windows PowerShell 执行器）把整条命令作为**一个** argv 元素传给 `-Command`，让 PowerShell 自己解析，因此不存在 `bash -c` 那层字符串引号域的问题；`TERM=dumb` 属于 POSIX（一套类 Unix 系统共同遵循的接口标准）世界的概念，在这里被刻意省略。源码用 `/* jscpd:ignore */` 明确标注：它是对 `bash-local` 的**刻意逐行镜像**（call-for-call），而非偷懒复制——两边有意保持一一对应，方便对照维护。`[verified]`（`packages/shell/pwsh-local/src/index.ts`）
...
社区普遍把 dsh 与 Claude Code / Codex CLI / Pi 归为同类 terminal/web agent harness（`[claimed]`，见《全网调研》E 节）。在"执行命令"这条线上，源码里留了几处直接对标同行的注释痕迹，正好让我们看清 dsh 在哪些地方选择"随大流"、又在哪些地方走自己的路。这些痕迹恰恰显示 dsh 是有意与同行收敛而非另起炉灶：`ENV_OVERRIDES` 的注释直言这是 "the same set Codex hardcodes; Claude Code achieves it via TERM=dumb"，`graceMs` 默认 3s 也 "matches OpenCode's 3s"（`packages/shell/bash-local/src/index.ts:27,35` `[verified]`；竞品内部细节仅为 dsh 注释转述、未独立核实，故 `[claimed]`）。换句话说，dsh 在环境、超时这类**无差异细节上对齐业界**，把独创性集中投在**分层可替换**上——这也正是它在 harness 可替换性目标下的取舍所在。
...
先厘清一处容易被当成缺陷的边界：持久 PTY 的能力范围是**记录在案的有意收敛**，而非遗漏。persistent-pty Agent Note 白纸黑字声明只支持交互 shell 与行式 REPL（Linux/macOS），而全屏终端应用、按键序列、BEL 触发的控制流、进程丢失后的会话恢复、跨 agent 会话共享都明确延迟——遇到全屏 TUI（Text User Interface，文本用户界面，即在终端里铺满整屏、靠光标绘制的程序，如 `htop`、`vim` 全屏模式）会碰壁，但那是范围声明，`bash/read/write/edit` 仍是可靠默认（`.agents/notes/implemented/feature/2026-07-16-persistent-pty-sessions.md` `[verified]`）。
