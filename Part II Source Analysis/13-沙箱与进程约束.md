...
协作点很实在：工具执行侧（第 12 章的 Consumer）把一次 `tools/pre-execute` 决策拆成 `allow`／`deny`／`ask` 三态，只有 `ask` 才去问审批服务、拿到 `allowed-once` 才放行`[verified]`（`packages/core/tools/src/index.ts:588,1706-1724`）。审批请求里的 `reason` 可承载"一个 hook 的许可决策原因"`[verified]`（`user-approval/src/index.ts:47-48`），这是它与第 18 章 hook 桥的接触面；而 ACP（Agent Client Protocol）桥提供机器答复者，只给"允许一次／拒绝"两个一次性选项，绝不从未知回应里推断持久授权`[verified]`（`packages/acp/acp/src/index.ts:271-283`）。
