...
协作点很实在：工具执行侧（第 12 章的 Consumer）把一次 `tools/pre-execute` 决策拆成 `allow`／`deny`／`ask` 三态，只有 `ask` 才去问审批服务、拿到 `allowed-once` 才放行`[verified]`（`packages/core/tools/src/index.ts:588,1706-1724`）。审批请求里的 `reason` 可承载"一个 hook 的许可决策原因"`[verified]`（`user-approval/src/index.ts:42`），这是它与第 18 章 hook 桥的接触面；而 ACP（Agent Client Protocol）桥提供机器答复者，只给"允许一次／拒绝"两个一次性选项，绝不从未知回应里推断持久授权`[verified]`（`packages/acp/acp/src/index.ts:215-228`）。
...
凭据的核心原则一句话：配置里只放"引用"（环境变量名），真正的值由 provider 保管`[verified]`（`packages/credentials/credentials/src/index.ts:54-58`）。消费方每次操作重新解析（resolve）、从不跨操作缓存，所以换了密钥无需重启就能在下一次请求生效——LLM 适配器就是每请求解析一次 bearer token`[verified]`（`credentials/src/index.ts:73`；`packages/llm/llm-deepseek/src/adapter.ts:58,83`）。`describe` 只回报"是否已配置／来源层／能否写入"，永不吐出值`[verified]`（`:81`）。不落日志的约束贯穿始终：本地 provider 的文档若被同组或其他用户可读（权限位非 `0600`）就直接拒绝启动，写入一律 `0600` 原子落盘`[verified]`（`packages/credentials/credentials-local/src/index.ts:103-122,394`）；解析出错时只打印行列号与错误码、绝不引用出错的那一行，因为那行就是密钥`[verified]`（`:154-186`）。这与第 17 章 settings 互补：settings 用 `role('secret')` 把声明为机密的字段在过线前结构化抹除`[verified]`（`packages/settings/settings/src/redact.ts:52-53`），而凭据从根上就不让密钥进配置文件。
