...
它的定位有一句被反复重申的边界声明写在模块 JSDoc 里：这是**同世界（same-world）子进程约束**——所谓"同世界"，指的是被约束的进程和宿主机共用同一个内核、同一套文件系统，只是在这套共享环境里划出一圈"哪些文件能碰"的围栏。容器、microVM（micro virtual machine，微型虚拟机，比传统虚拟机更轻量的一种隔离）、远程执行**不是**这条接缝的后端，它们做的是另一件事：整体换掉 `ctx.shell`、`ctx.fs` 这些能力接缝的 Service Provider，把 agent 整个搬进另一个环境 `[verified]`（`index.ts:1-6`、`sandbox.md:5`）。为什么不能混用？理由很实在：一个 agent 若 bash 跑在容器里、fs 工具却写宿主盘，它读到的和写出的就对不上，等于活在两个割裂的世界里。
...
- **fallback 必须自带**：首选 runner `bwrap` 偏偏在最需要沙箱的宿主上最容易缺席（精简过的容器、禁用了非特权 userns 的系统、拒绝 `mount` 的 LSM（Linux Security Module，Linux 安全模块，内核里负责访问控制的一层）安全策略）。所以 SDK（Software Development Kit，软件开发工具包）必须**自带**一个后备（fallback）启动器，而不能指望宿主机上正好装了 `[verified]`（`2026-07-06-sandbox.md:11`）。
...
**后端链**在 `dsh-sandbox-local` 里就是一张按平台索引的表：Linux 是 `['bwrap', 'landlock']`（Landlock 是 Linux 内核自带的一套沙箱机制，让进程给自己装一份"哪些路径可读可写"的规则），darwin 是 `['seatbelt']`（Seatbelt 是 macOS 系统级的沙箱机制，靠 `sandbox-exec` 加载一份 profile 来约束进程），win32 是 `['windows-acl']`（靠 Windows 的 ACL——Access Control List，访问控制列表——加受限令牌来约束）`[verified]`（`sandbox-local/src/index.ts:159-166`）。选择的逻辑是**先按平台定候选、再按探测挑一个**：一个平台有多个候选时，就按顺序做**功能性探测**——注意是"真的建一个 profile 并强制执行一次、看它到底管不管用"，而不是查一下 `--version` 就信。为什么单候选反而直接选、不探测？因为探测的作用是"从多个候选里仲裁出一个能用的"，只有一个候选时根本没什么可仲裁，而探测本身有成本，会拖慢每个 session 的第一条受限命令 `[verified]`（`index.ts:492-539`、`2026-07-06-sandbox.md:136`）。
...
- `enforcement: 'full' | 'partial'`——后端对本次策略承诺的文件效果，到底是**全部都管住了**，还是**只管住其中一个子集**（比如旧内核 ABI（Application Binary Interface，应用二进制接口，即内核与程序之间那套底层调用约定）或某些后端能力有限）。这相当于一张诚实的自我标注：要求绝对边界的 caller **不得**把 `partial` 当 `full` 用。
...
**native 启动器**是这条链上最"贴地"的一块。`native/landlock-run` 是一个约 **298 行的 C 程序**（纯 C11 直接调用 Landlock 的 UAPI（User API，内核开放给用户态程序直接调用的接口）内核接口，除了静态链接进去的 musl（一个轻量的 C 标准库实现）之外不依赖任何库，所以要审计它，需要看的就是这一个文件加上内核那套稳定的 syscall 契约——审计面小到几乎可以逐行读完）。它干的事概括成一句话是"**先把自己关起来，再 exec 目标命令**"：命令行上用 `--ro <path>` / `--rw <path>` 声明"这些路径只读、那些路径可写"，`--` 后面跟着真正要跑的 argv；它先设置 `no_new_privs`（禁止后续再提权）、把 ruleset（授权规则集）装到自己身上，然后才 `exec` 目标命令——关键在于 ruleset 会跨 `execve` 被子进程继承下去，所以"先自缚"等于"约束了之后的一切" `[verified]`（`2026-07-06-sandbox.md:65`、`main.c` 298 行实测、`native/landlock-run/docs/architecture.md:22-24`）。

启动器打包成"一个 entry 包 + 每平台一个二进制包"的家族：entry 包是纯 ESM（ECMAScript Modules，JavaScript 的标准模块格式）JS，握着 CLI（Command-Line Interface，命令行界面）契约（`launcherPath`、`probe`、常量），并在 tarball 里附带 C 源码供审计；每个平台包则只装一个预编译好的静态二进制、不含任何 JS，靠 npm 的 `os`/`cpu` 字段在安装时自动挑中匹配当前系统的那个 `[verified]`（`architecture.md:5-18`）。为什么要这么拆？因为这样 CLI 解析器与二进制被绑在**同一个版本家族**里一起发布，解析器永远不会比二进制旧——这正是拆分存在的唯一理由。此外 dsh 刻意**不把编译好的二进制提交进仓库**（二进制在 diff 里没法审、还会把提交历史搅乱），改用"审 C 源码 + 在原生 CI（Continuous Integration，持续集成，即每次改动都自动构建、测试的流水线）上构建 + 主仓库逐字节 pin 住的发布演练"这一整套来保证可信 `[verified]`（`2026-07-06-sandbox.md:137`、`architecture.md:28`）。
...
前七节讲的是"进程能做什么"的硬约束（沙箱、Landlock、进程树）。这一节补上三个软性但同样属于安全域的能力：审批（approval，谁批准一个动作）、权限预设（permission-presets，把安全开关打包成一个选项）、凭据（credentials，密钥怎么存取而不外泄）。它们都不在 agent 主循环骨架上，而是可选插件，各守一条清晰边界。

### 8.1 审批流：一个动作能不能往下走

审批只回答一句话：这个具体动作可以继续吗？结果被收敛成四个封闭取值 `allowed-once`／`rejected`／`cancelled`／`unavailable`，其中只有 `allowed-once` 是"放行一次"，其余三个调用方一律当作拒绝——这就是"失败即关闭（fail-closed）"`[verified]`（`packages/interaction/user-approval/src/index.ts:82`）。答复者（answerer）抛异常、缺席或返回非法值，都会被规整成 `unavailable` 而非放行`[verified]`（`user-approval/src/index.ts:325,328`）。每个会话有一条策略：`ask` 走答复者瀑布链，`never` 则在派发前就确定性地返回 `rejected``[verified]`（`:94,312`）——无人值守的 CI（continuous integration，持续集成）批处理设成 `never`，任何需批准的动作直接被拒。

协作点很实在：工具执行侧（第 12 章的 Consumer）把一次 `tools/pre-execute` 决策拆成 `allow`／`deny`／`ask` 三态，只有 `ask` 才去问审批服务、拿到 `allowed-once` 才放行`[verified]`（`packages/core/tools/src/index.ts:588,1706-1724`）。审批请求里的 `reason` 可承载"一个 hook 的许可决策原因"`[verified]`（`user-approval/src/index.ts:41-42`），这是它与第 18 章 hook 桥的接触面；而 ACP（Agent Client Protocol）桥提供机器答复者，只给"允许一次／拒绝"两个一次性选项，绝不从未知回应里推断持久授权`[verified]`（`packages/acp/acp/src/index.ts:271-283`）。

### 8.2 权限预设：把两个开关打包成一个选项

权限预设不拥有任何执行力，它只把两个独立开关——沙箱模式（`sandbox/mode`，本章前文）与审批策略（`approval/policy`）——捆成一个命名选项，供客户端当作单一的"Permissions"下拉框`[verified]`（`packages/interaction/permission-presets/src/index.ts:55-64`）。默认两项：`workspace-write`（工作区可写 + `ask`）与 `danger-full-access`（完全访问 + `never`）`[verified]`（`:167-176`）。切换预设时先记一条日志型 `permission/preset` 事件，再通过各自 setter 写回两个开关、且只写真正变化的那个`[verified]`（`:375-392`）——这样"用户选了哪个预设"这份意图可回放，即使两个预设的开关组合恰好相同 `[inferred]`。若挂载的 bash 执行器不具备约束能力（无 `sandboxMode`），插件在加载期就报错`[verified]`（`:189-194`），这是它压在沙箱之上的硬性前置。

### 8.3 凭据：引用进配置，值不落配置

凭据的核心原则一句话：配置里只放"引用"（环境变量名），真正的值由 provider 保管`[verified]`（`packages/credentials/credentials/src/index.ts:1-9,54-58`）。消费方每次操作重新解析（resolve）、从不跨操作缓存，所以换了密钥无需重启就能在下一次请求生效——LLM 适配器就是每请求解析一次 bearer token`[verified]`（`credentials/src/index.ts:73`；`packages/llm/llm-deepseek/src/adapter.ts:58,83`）。`describe` 只回报"是否已配置／来源层／能否写入"，永不吐出值`[verified]`（`:81`）。不落日志的约束贯穿始终：本地 provider 的文档若被同组或其他用户可读（存在任何 group/other 权限位，即 `0o077` 掩码命中）就直接拒绝启动，写入一律 `0600` 原子落盘`[verified]`（`packages/credentials/credentials-local/src/index.ts:103-122,394`）；解析出错时只打印行列号与错误码、绝不引用出错的那一行，因为那行就是密钥`[verified]`（`:154-186`）。这与第 17 章 settings 互补：settings 用 `role('secret')` 把声明为机密的字段在过线前结构化抹除`[verified]`（`packages/settings/settings/src/redact.ts:52-53`），而凭据从根上就不让密钥进配置文件。
...
%%{init: {'theme':'neutral'}}%%
flowchart LR
  Preset["权限预设<br/>permission/preset"] -->|写回| Sandbox["沙箱模式 Ch13"]
  Preset -->|写回| Policy["审批策略<br/>ask/never"]
  Tool["工具 Consumer Ch12"] -->|ask| Approval["审批服务"]
  Policy -.约束.-> Approval
  Hook["hook 桥 Ch18"] -.reason.-> Approval
  ACP["ACP 桥"] -->|答复者| Approval
  Approval -->|allowed-once| Tool
  Cred["凭据 引用→值"] -->|每请求解析| LLM["LLM 适配器"]
  Settings["settings Ch17"] -.只存引用.-> Cred
```

</div>

> 图注：预设是"打包器"，写回沙箱与审批两个开关；审批被工具 Consumer 触发、受策略约束、由 hook/ACP 供给答复；凭据与 settings 各守一边——密钥不入配置、不落日志。

**补充源码索引（审批/权限/凭据）**
- 审批：`packages/interaction/user-approval/src/index.ts:82`（结果取值）、`:94/312`（`never` 前置短路）、`:257-276`（请求+审计对）、`:325/328`（非法答复规整）
- 审批消费方：`packages/core/tools/src/index.ts:588`（决策三态）、`:1706-1724`（映射 allow/deny）
- 权限预设：`packages/interaction/permission-presets/src/index.ts:55-64/167-176`（打包与默认）、`:375-392`（切换写回）、`:189-194`（加载期校验）
- 凭据：`packages/credentials/credentials/src/index.ts:54-58/73/81`；本地 provider `packages/credentials/credentials-local/src/index.ts:103-122/154-186/394`
- 与 settings 脱敏互补：`packages/settings/settings/src/redact.ts:52-53`
- 子系统文档：`docs/subsystems/{approval,permission-presets,credentials}.md`
...
- `packages/sandbox/sandbox/src/roots.ts:1-55` — `writableRoots` / `canonicalPath`：workspace-write 的规范化可写根
...
- `native/landlock-run/docs/architecture.md` + `native/landlock-run/packages/entry/src/main.c`（298 行）— 自约束再 exec 的 C 启动器、两层包家族、功能性探测
