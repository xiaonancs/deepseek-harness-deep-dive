...
**schedule（定时提醒）。** `ScheduleId` 同为品牌化、会话内不复用。三种规则记录各管一类：`After`（正延时一次性）、`At`（绝对时刻一次性）、`Every`（固定频率、最短五分钟）（`packages/schedule/schedule/src/types.ts:13`、`:27`、`:39`）。创建时统一把首个目标规范化为四位年份的 RFC 3339（互联网时间戳标准）UTC 时刻，重放不依赖任何环境时区。它是**会话本地的耐久提醒**：投递模式恒为 `session-local`（`types.ts:111`），到点不走任何外部通道，而是等 agent 完全空闲、进入维护相后 `followup()` 一条普通用户消息，作为一个寻常对话轮回到原活会话（`packages/schedule/schedule/src/runtime.ts:275`）——从不 `steer()`、绝不打断当前轮。

**user-questions（问答）。** `ctx.userQuestions` 是一套 provider 中立的词汇：当某个工具或权限插件必须先让人回答、agent 才能往下走时用它（`packages/interaction/user-questions/src/index.ts:51`）。`ask()` 只对"注册表认定的、恰好在世的运行时根"放行，被别的 agent 拥有的子实例会得到 `CALLER_NOT_LIVE`／`DELEGATED_CALLER`（`:92`、`:105`），避免子代理卡死在无人应答的问题上。它与 Ch13 的审批、本章 plan 是同一族——`plan-review` intent 正是借这条缝把计划交给人裁决。
...
- `docs/subsystems/schedule.md`（durable 记录:7、时区边界:67、批处理/追赶:92、活投递:180）
- `packages/schedule/schedule/src/types.ts:10,13,27,39,111`；`runtime.ts:255-280`
- `docs/subsystems/user-questions.md`（provider 中立词汇:5、presentation intent:24、活根边界:75）
- `packages/interaction/user-questions/src/index.ts:43,51,64,92,105`
