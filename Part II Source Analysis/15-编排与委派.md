...
4. **模型自己写的编排脚本会不会卡死或搞垮宿主？** → 放进 worker 线程 + vm context 里跑，一次 run 配一个 worker，跑飞了可以强制终止（但文档明确说这"是隔离而非安全边界"，`workflow-worker-thread/src/index.ts:1`——意思是它能防脚本卡死宿主，但不能用来跑不可信代码）。
...
`tool-workflow` 给模型看的那段工具描述，本身就是一份脚本编写规范：脚本用纯 JS（不是 TS）、允许 top-level await（顶层直接 await，不必包一层 async 函数）、以 `return <json>` 结尾；能用的钩子只有五个——`agent()`/`pipeline()`/`parallel()`/`phase()`/`log()`，而且**碰不到文件系统、网络、定时器、Node API**。一句话概括这个分工："agent 才是干活的，脚本只负责调度协调"（`tool-workflow/src/index.ts:138-150`）。`meta`/`args` 是纯 JSON 数据（脚本的元信息和入参），引擎在执行任何一行脚本文本之前，先校验 `meta` 的 schema，不合格就直接硬拒（`docs/subsystems/workflow.md:13`）——把配置和代码分开、先验配置，是为了让错误尽早暴露。
...
在编排风格上，dsh 没有选逐轮委派（模型每步决定下一个委派）、也没有选声明式 DAG，而是让模型直接写一段命令式 JS 编排脚本。之所以如此，是因为 `WorkflowMeta` 的字段词汇明确"匹配 Claude Code dynamic-workflows meta block"（`docs/subsystems/workflow.md:41`，[verified]），本就是有意对标 Claude Code 的动态工作流思路；命令式脚本让模型复用既有编程能力，`pipeline` 的无 barrier 语义又正好为"几十上百文件审计"这类大规模独立 fan-out 优化——这些是逐轮委派会撞上回合开销、DAG 又表达不了动态分支的地方。
