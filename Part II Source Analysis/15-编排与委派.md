...
<p>图注：subagent 是唯一"多 provider 并存"的接缝；workflow/ralph 建立在它之上；jobs 把 subagent 当作后台生产者之一；plan 与 todo 是独立的记录型能力。此图证明五个子系统的依赖方向与角色划分。</p>
...
<p>图注：`running`/`waiting`/`settled` 由 Agent 静默性与 owned-child 集合派生，不额外维护状态机；`followup` 三种落点（同 Activation 入队 / 唤醒 / 冷恢复）由此表决定。此图证明续跑层"把顺序完全托管给 inbox"的设计。</p>
...
`interrupt()` 是唯一公开的停止操作：它同步做完授权检查、发出 `Agent.cancel(cause,{keepInbox:true})`、然后立即返回，不等子 agent 真正安静下来；那些还没被认领的 inbox 工作、Activation、以及已经发布出去的后代都会保留（`docs/subsystems/subagent.md:144`）。`listChildren()`/`listDescendants()` 则是**纯只读的枚举**，只从"会话存储 + 可选持久化"的"活优先"合并视图里读，绝不加载或恢复任何 Agent；它靠一个三级阶梯来解析每一行的 `mode`/`label`：先查活子的 watermark（进度水位线，记录已读到哪）缓存，查不到再看冷子的投影 checkpoint，还不够才落到一次 `persistence.inspect()`（`docs/subsystems/subagent.md:289`）。这种"能省则省"的阶梯设计，是为了让"列个清单"这种高频操作尽量不去唤醒沉睡的子 agent。
...
<p>图注：脚本在 worker 线程的 vm 里跑，`agent()` 通过 RPC（Remote Procedure Call，远程过程调用，即"像调本地函数一样去调另一处的代码"）回到宿主经 subagent 接缝启动子 agent；脚本的返回值跨 worker 边界前被"材质化"为纯 JSON。此图证明 workflow 与 subagent 是"引擎—接缝"的组合关系，而非各自造轮子。</p>
...
<p>图注：plan 状态从"挂起选择"到"日志事件"再到"提示段"，全程以日志为唯一真源；恢复/分叉/压缩都只重放折叠。此图证明 plan 遵守总纲"模型可见 ⟺ 已记录"不变量（Ch07）。</p>
...
- **委派深度**：在 `SessionHeader.delegationDepth` 与运行时的 `AgentOptions.subagentDepth` 之间取更大的那个（是取较大值，不是相加）；这两个字段被接缝独占，核心 loop 既不设置也不读取，而且冷恢复时深度只能不变或更深、不能被调低（`docs/subsystems/subagent.md:467`）——这样能防止有人靠恢复来绕过深度限制。
...
## 八、补充能力域：会话目标·定时提醒·问答·命令

前七节讲的 subagent、workflow、plan、jobs、todo，都是"把一段活儿如何铺开"的编排原语。这一节再补四个同处编排／交互层、却各守一条更窄边界的能力：它们都是**长在 agent 之外、绕着会话展开的可选插件**——不动 `agent-loop`，凡模型可见者皆可从会话日志重建（呼应 Ch07、Ch11），但一个管"要往哪儿去"，一个管"到点回来一趟"，一个管"先问人再走"，一个管"人直接按一下"。

**goal（会话目标）。** `GoalId` 是品牌化 id，`GoalRef` 把"身份 + 修订号"绑成一个比较并置换（compare-and-set）句柄：每次落地变更都让 `revision` 自增，改错版本直接拒绝（`packages/goal/goal/src/types.ts:16`、`:19`）。目标状态是纯事件溯源——`GoalPhase` 有 `active/paused/blocked/complete` 四相（`types.ts:44`），每次变更都写一条 `goal/change` 日志，`GoalService` 严格重放折叠出当前视图（`packages/goal/goal/src/index.ts:183`）。"是否可自动续跑"这件事是进程本地的 activation，不进日志；被准入的每个续跑回合则由 `GoalMessageSource` 记上一个正的、连续的 `round` 号，实现轮次归属（`packages/goal/goal/src/domain.ts:47`）。变更落定后经 `goal/changed` 做 scoped 通知（`domain.ts:114`）。

**schedule（定时提醒）。** `ScheduleId` 同为品牌化、会话内不复用。三种规则记录各管一类：`After`（正延时一次性）、`At`（绝对时刻一次性）、`Every`（固定频率、最短五分钟）（`packages/schedule/schedule/src/types.ts:13`、`:27`、`:39`）。创建时统一把首个目标规范化为四位年份的 RFC 3339（互联网时间戳标准）UTC（协调世界时）时刻，重放不依赖任何环境时区。它是**会话本地的耐久提醒**：投递模式恒为 `session-local`（`types.ts:111`），到点不走任何外部通道，而是等 agent 完全空闲、进入维护相后 `followup()` 一条普通用户消息，作为一个寻常对话轮回到原活会话（`packages/schedule/schedule/src/runtime.ts:275`）——从不 `steer()`、绝不打断当前轮。

**user-questions（问答）。** `ctx.userQuestions` 是一套 provider 中立的词汇：当某个工具或权限插件必须先让人回答、agent 才能往下走时用它（`packages/interaction/user-questions/src/index.ts:51`）。`ask()` 只对"注册表认定的、恰好在世的运行时根"放行（`ask()` 签名在 `:92`），被别的 agent 拥有的子实例会得到 `CALLER_NOT_LIVE`（`:105`）／`DELEGATED_CALLER`（`:111`），避免子代理卡死在无人应答的问题上。它与 Ch13 的审批、本章 plan 是同一族——`plan-review` intent 正是借这条缝把计划交给人裁决。

**commands（命令）。** `ctx.commands` 是人类命令的注册表：交互式适配器用它发现并**直接执行**插件自带的命令，全程不产生任何模型消息（`packages/interaction/commands/src/index.ts:225`）。`execute()` 解析斜杠命令行后就地跑 handler，只在日志里补 `command/run` / `command/done` 一对生命周期事件、不包在任何回合里（`:297`）；注册表增删则以 `commands/change` 广播给各 UI（用户界面）视图（`packages/interaction/commands/src/types.ts:72`）。
...
flowchart TD
  LOG["会话日志 事件溯源真源"]
  subgraph G["goal 会话目标"]
    G1["GoalRef 比较并置换"]
    G2["goal/change 折叠"]
    G3["round 轮次归属"]
  end
  subgraph S["schedule 定时提醒"]
    S1["After / At / Every"]
    S2["到点 followup 普通轮"]
  end
  subgraph U["user-questions 问答"]
    U1["ctx.userQuestions.ask"]
    U2["仅活运行时根"]
  end
  subgraph C["commands 命令"]
    C1["ctx.commands.execute"]
    C2["不产生模型消息"]
  end
  G --> LOG
  S --> LOG
...
<p>图注：goal 与 schedule 把状态完全托付给日志折叠（模型可见 ⟺ 已记录）；commands 只补一对生命周期事件；user-questions 不进日志，靠阻塞等人应答。四者同处编排／交互层，却分别守住"目标身份""到点回轮""先问后行""人直接触发"四条互不重叠的边界。</p>

</div>

### 补充源码索引（本节）

- `docs/subsystems/goal.md`（身份/生命周期:7、durable 变更:72、GoalMessageSource 轮次:100、GoalService:145）
- `packages/goal/goal/src/types.ts:16,19,44,59,74`；`domain.ts:47,114`；`index.ts:183,557`
- `docs/subsystems/schedule.md`（durable 记录:7、时区边界:90、批处理/追赶:92、活投递:180）
- `packages/schedule/schedule/src/types.ts:10,13,27,39,111`；`runtime.ts:255-280`
- `docs/subsystems/user-questions.md`（provider 中立词汇:5、presentation intent:24、活根边界:73）
- `packages/interaction/user-questions/src/index.ts:43,51,64,92,105,111`
- `docs/subsystems/commands.md`（注册表定位:5、定义:22、生命周期日志:159）
- `packages/interaction/commands/src/index.ts:225,245,297,308`；`types.ts:72`
...
> **rc.6–rc.7 更新（截至 2026-08-17）**：新增 ADR `2026-08-12-product-subagent-one-shot-background-tasks`，让 **Codex 与 Claude Code 两个子代理后端能作为"后台一次性任务"运行**（`feat(agent-presets)`，PR #2374），fork 出来的子代理仍保持 one-shot 语义（`2026-08-10-fork-children-stay-one-shot`）`[verified]`。这是本章"六个 provider、one-shot vs continuable"叙述的延伸：此前 Codex/Claude Code 后端偏同步一次性，现在可排到后台跑。详见 [Appendix/更新追踪](../Appendix/更新追踪.md)。
