...
与第 16 章前半的衔接由此清晰：`compaction-basic` 注入 `tokenMeter` 并反复 `measure()`（`packages/compaction/compaction-basic/src/index.ts:104,267`），拿 `totalTokens` 比对 `thresholdRatio` 判断"该不该压"，再拿 `nodes` 切出可压区间做规划（`src/region.ts:344,420`）。度量器刻意保持"读"的纯粹：脱离态、深冻结、O(surface) 克隆，压缩才是那个"写"的一方。这也解释了为何固定启发式的误差（CJK 文本、JSON schema 在"四字符一 token"下会低估）被接受为已知局限（`README.md:65-67`）——它服务的是"是否越过阈值"这类相对判断，精确容量归 `ctx.llm.resolveModelInfo().context` 管。

### 补充源码索引（本节）

- 度量服务与两方法：`packages/llm/token-meter/src/index.ts:74,116,155`；固定启发式：`src/estimate.ts:13,19,26`
