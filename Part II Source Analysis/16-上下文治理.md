...
- **压缩的边界只保配对不保整回合**：区间边界用 `toolPairingBalancedBefore/After` 保证工具调用/结果成对（不会把"调用"和"结果"拆散），但它不保护"整回合"——失控回合内那些早已闭合的步骤照样会被压缩（`compaction.md:86`）。如果撑爆预算的是单个超大的不可分单元，或是请求信封（一次请求里除历史之外的固定开销，如系统提示与工具 schema）本身，压可见面也救不了。
...
## 八、补充能力域：token 计量与预算（token-meter）

前面反复出现一个角色：压缩靠"度量"决定何时该动手。这个度量器就是 `@deepseek-ai/dsh-token-meter`（`ctx.tokenMeter`）。token 可理解为"模型计价与上下文占用的最小单位"——桌子还剩多少地方、这次请求要付多少租金，全靠它报数。它的定位很克制：**一个脱离态（detached）的重放快照，度量请求压力（request pressure）与可见面（surface）的定位定价**，本身既不压缩也不改日志。

它对外只有两个方法（`packages/llm/token-meter/src/index.ts:116,155`）：`measure(session, requestHeader?)` 返回一次快照，`estimateMessage(message)` 按固定启发式给单条消息定价。快照类型 `TokenMeasurement`（`src/types.ts:20-34`）有六个字段——`logRevision`（已消费的持久事件数，等于下一个待读 seq，`types.ts:23`）、`baseline`（锚点，三态 `none | estimated | usage`，`types.ts:15-18`）、带符号的 `surfaceDeltaTokens`、非负的 `totalTokens`、`surfaceTokens`，以及按头到尾顺序排列的 `nodes`（每个 `TokenSurfaceNode` 只含 `seq` 与 `tokens`，`types.ts:37-42`）。

关键在"计量读的是日志派生的 surface"：`measure()` 把会话事件流回放到当前持久尾部（`index.ts:174-179`），对其中的可见面事件调 `deriveEventMessage` 取出消息、经 `foldSurfaceTokens` 逐条定价（`src/surface-fold.ts:42-64`，`append` 追加节点、`replace` 按区间替换并给出带符号 delta）。定价用一套固定启发式——四字符约等于一个 token，外加角色与块的结构开销（`src/estimate.ts:13,16,19`）。也就是说，surface 不是它自己维护的状态，而是 Ch07 那条 append-only 会话日志派生出来的投影；快照是不可变的，底层 fold 前进时旧快照不会跟着长（`README.md:18`）。锚点的取舍是它最见功力的一处：只有当最近一次成功调用的规范请求信封与本次匹配、且其 provider 用量不低于同一信封的完整启发式价时，才复用真实用量（`kind: 'usage'`），否则整体退回启发式估算（`index.ts:246-248`）；`surfaceDeltaTokens` 带符号，因此增长与收缩都能相对锚点保留。
...
%%{init: {'theme':'neutral'}}%%
flowchart LR
  Log["会话日志 append-only surface 事件"] -->|"deriveEventMessage"| Fold["foldSurfaceTokens 逐条定价"]
  Fold --> Snap["TokenMeasurement 脱离态快照"]
  Snap -->|"nodes plus totalTokens"| CMP["compaction-basic 判定何时压"]
  CMP -->|"replace 旧区间"| Log
```

</div>

> 图注：计量器把日志的可见面事件折叠成一份不可变快照；compaction 读它的压力与节点来决定是否压缩、压哪一段。此图证明"度量"与"压缩"是读写分离的两方——meter 只报数，改日志的永远是 compaction。

与第 16 章前半的衔接由此清晰：`compaction-basic` 注入 `tokenMeter` 并反复 `measure()`（`packages/compaction/compaction-basic/src/index.ts:104,267`），拿 `totalTokens` 比对由 `thresholdRatio` 换算出的 token 阈值判断"该不该压"，再拿 `nodes` 切出可压区间做规划（`src/region.ts:344,420`）。度量器刻意保持"读"的纯粹：脱离态、深冻结、O(surface) 克隆，压缩才是那个"写"的一方。这也解释了为何固定启发式的误差（CJK（中日韩）文本、JSON schema 在"四字符一 token"下会低估）被接受为已知局限（`README.md:34、65-67`）——它服务的是"是否越过阈值"这类相对判断，精确容量归 `ctx.llm.resolveModelInfo().context` 管。

### 补充源码索引（本节）

- 度量服务与两方法：`packages/llm/token-meter/src/index.ts:74,116,155`；固定启发式：`src/estimate.ts:13,16,19`
- 快照类型：`src/types.ts:15-18,20-34,37-42`（`baseline` 三态、`TokenMeasurement`、`TokenSurfaceNode`）
- 日志派生的可见面 fold：`src/surface-fold.ts:42-64`；子系统页与 Cordis 目录：`docs/subsystems/token-meter.md`
- 与压缩的读写衔接：`packages/compaction/compaction-basic/src/index.ts:104,267`、`src/region.ts:344,420`；近似性说明：`packages/llm/token-meter/README.md:18,65-67`
