...
`agent-instructions` 的**触碰驱动刷新**（没有文件监视器，外部编辑要等你下次成功调用文件工具、或 resume 重新对账才可见）应看作刻意取舍而非缺陷：解析任意 shell `cd` 并不可靠、每个 bash 调用都是新 shell，远程/沙箱工作区也没有可靠的 host 文件监视，触碰驱动反而让"何时刷新"可预测、可测；代价是保留一段窗口期陈旧作为已知局限（`agent-instructions/README.md:164-166`，`[verified]`）。
