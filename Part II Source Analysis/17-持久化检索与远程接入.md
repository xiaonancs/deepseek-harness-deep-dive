...
`ctx.attachments` 是抽象服务 `AttachmentStore`，对外暴露三个抽象方法 `validateImage`、`saveImage`、`readImage`，外加一个公开的批量入口 `saveImages` 与只读的 `imageLimits` `[verified]`（`packages/attachment/attachment/src/index.ts:29-74`）。枢纽是类型 `ImageAttachmentRef`——一个可序列化、按内容寻址（content-addressed，即用内容摘要而非路径来标识）的引用，其 `attachmentId` 注释直言"永远不是文件系统路径或 bearer URL"`[verified]`（`src/types.ts:11-24`）。也就是说，会话事件里绝不落 browser 的 object URL、宿主临时路径、provider URL 或 base64，只存这枚小小的引用 `[verified]`（`docs/subsystems/attachment.md:5`）。这好比行李寄存：你拿到的是一张编号存根，而不是把整只箱子塞进登记簿。
...
**补充源码索引（附件）**
- `packages/attachment/attachment/src/index.ts:29-60` — `ctx.attachments` 接缝与三方法契约
...
- `packages/acp/acp/src/index.ts:169-176,241` — ACP 图像块的文本标记降级（rc.7 桥的前身）
