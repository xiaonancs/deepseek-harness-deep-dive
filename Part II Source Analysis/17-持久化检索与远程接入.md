...
`ctx.attachments` 是抽象服务 `AttachmentStore`，对外只暴露三个方法：`validateImage`、`saveImage`、`readImage` `[verified]`（`packages/attachment/attachment/src/index.ts:29-60`）。枢纽是类型 `ImageAttachmentRef`——一个可序列化、按内容寻址（content-addressed，即用内容摘要而非路径来标识）的引用，其 `attachmentId` 注释直言"永远不是文件系统路径或 bearer URL"`[verified]`（`src/types.ts:11-24`）。也就是说，会话事件里绝不落 browser 的 object URL、宿主临时路径、provider URL 或 base64，只存这枚小小的引用 `[verified]`（`docs/subsystems/attachment.md:5`）。这好比行李寄存：你拿到的是一张编号存根，而不是把整只箱子塞进登记簿。

三个方法边界很干净 `[verified]`（`attachment-local/src/store.ts:63-231`）：`validateImage` 只跑准入策略、完整解码校验，一个字节都不落盘；`saveImage` 才真正提交，且注释要求"在其所属会话事件被追加**之前**"完成（`index.ts:50`）；`readImage` 按内容寻址重算摘要核对，全程尊重取消信号——多处 `signal?.throwIfAborted()` 把 abort 原样抛出，而不是伪装成一次存储失败（`store.ts:209-225`）。最见功力的是**有序图像批次准入**：批量写先对每个成员逐一 `validateImage`，全部通过后才按序 `saveImage`，这样一张坏图也不会把先前已写入的成员留成无人引用的孤儿对象 `[verified]`（`index.ts:37-40`）。

图片进入模型的姿势很克制：它化身角色中立的核心 `ImageBlock`（只裹一个 durable ref，见第 10 章内容块词汇），再由 provider 适配器在请求时解析出真实字节 `[verified]`（`packages/llm/llm/src/types.ts:71-105`）。加图会改写 provider 请求、使受影响的请求后缀在 KV cache（key-value cache，键值缓存）里失效 `[verified]`（`README.md:13-15`）——这也解释了为何贴图会让这一轮略慢。落地实现是本地 provider `LocalAttachmentStore`，把对象按摘要存到 `DSH_HOME/attachments/v1` 下，v1 只收 PNG/JPEG/WebP/GIF `[verified]`（`attachment-local/src/index.ts:38-59`）；未发送的合成器图片始终是 browser-owned 临时草稿，重载即失。保留与垃圾回收（garbage collection，简称 GC）被 deferred——因 resume/fork 会话可能共享同一份不可变对象，按年龄删除可能误伤仍被引用的历史 `[verified]`（`docs/subsystems/attachment.md:72`）。至于 ACP（第四节的自动化协议）侧，当前源码把图像块降级成 `[image attachment …]` 文本标记、`promptCapabilities.image` 仍为 `false` `[verified]`（`packages/acp/acp/src/index.ts:169-176,241`），这正是本章"rc.6–rc.7 更新"注记里 ACP 图像桥要接续补齐的一环。
...
rc.7 的**插件自有设置面**（PR #2404，`feat/plugin-owned-settings-surface`）让每个注册命名空间在 Web 设置里出一张卡片：`ui-settings` 声明槽位，`ui-settings-plugins`/`-plugin-inventory`/`-models` 等客户端各自贡献页面，卡片按字段标注"是否被用户覆盖"并支持一键重置回 composition 默认 `[verified]`（`packages/client/ui-settings-plugins/README.md:5,17-25`）。
