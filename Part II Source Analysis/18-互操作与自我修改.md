...
## 五、与 Ch03 的呼应：自我修改是Spatiotemporal Composability 最直接的体现
...
把"让 agent 改写自己运行时"放进竞品坐标看，它更像 dsh 相对 Claude Code / Codex 的一项差异化能力——把"一切皆插件"贯彻到模型可自反修改运行时，mount 物还与框架插件同演算、可检视可回滚。但措辞需克制：源码能证明的是**机制存在且与Spatiotemporal Composability 同源**（机制 `[verified]`，`packages/extensions/README.md`、`cordis-host-runner/README.md`），而"是否更优"社区评价多为 `[claimed]`/`[inferred]`（《全网调研-社区认知地图》D/F 节）、无一手定论，也无法证明它在真实任务上净收益为正——这也是它至今停留在 opt-in 开发工具、而非进默认的原因之一。
