...
> **rc.6–rc.7 更新（截至 2026-08-17）**：官方已在动手缩小这块波及面——rc.7 新增两道门禁裁剪"装机面"（install face，即 `npm install` 时真正会拉取/加载的部分）：一是**拒绝在模块作用域静态 import 可选依赖**，二是**把仅浏览器用的依赖挡在装机面之外**（两道检查随后并入现存门禁 `scripts/verify-runtime-closure.ts`；引入它们的 commit `7b973e27c8`/`6e77499326` 因仓库历史改写已非当前 HEAD 祖先），并移除了 web 的模块预加载 polyfill `[verified]`。这印证了"阶段性局限"的判断：体积/供应链正被主动收敛，而非放任。详见 [Appendix/更新追踪](../Appendix/更新追踪.md)。
