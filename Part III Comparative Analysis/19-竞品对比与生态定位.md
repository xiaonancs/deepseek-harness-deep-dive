...
- **npm 供应链**：对深度依赖 npm（Node.js 的公共包仓库与包管理器，第三方库大多从这里安装）生态所带来的供应链风险的一般性担忧 `[claimed]`——即"我依赖的第三方包哪天被投毒或跑路怎么办"。dsh 的缓解姿态之一是把关键底座 vendored（把依赖钉死在某个具体的 Git 提交哈希上，即 pin SHA——SHA 是 Git 给每次提交算出的唯一指纹；再附一份本地改动清单，见 `AGENTS.md` 末节 Vendoring policy），相当于把最要紧的那层零件搬进自家库房、不随上游漂移；但这只覆盖 Cordis 一层，并非全量。
