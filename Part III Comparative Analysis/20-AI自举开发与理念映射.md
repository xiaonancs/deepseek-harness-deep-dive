...
一句话概括：`dsh` 要解决的难题，不是"怎么让一个 agent 写对一段代码"，而是"怎么让大量并行、主要由 agent 驱动的提交在两个月里协作出一个 219 包、可发布的系统而不塌方" `[verified]`（219 包见 `find packages -mindepth 3 -maxdepth 3 -name package.json`）。前者是能力问题，后者是治理问题——本章讲的正是后者。
...
- **Agent Note 不是可以事后补的作文**。它必须和代码在同一个 PR 里落地，而且 `implemented` 记录要跟"实际发布了什么"保持同步——一旦代码改了路径、包名或默认值，就得在同一个改动里顺手更新 Note（注意：只改对不上的事实，不动当初的决策）`[verified]`（`.agents/notes/implemented/CLAUDE.md`、`.agents/notes/README.md:13`）。
...
- **开源即贡献**：`dsh` 以 MIT 许可证（一种最宽松的开源许可证，几乎只要求保留版权声明）的开发者预览版开源；它把 Cordis 以 vendoring 方式纳入（vendoring 指把第三方库的源码直接收进自己仓库、连同改动一起管理）、并公开逐条列出对上游的本地改动——这与"要做贡献者而不是搭便车者、开源本身是声誉/人才/标准层面的战略"这一立场**呼应** `[verified 事实]`（`AGENTS.md:147-149` vendoring policy：钉住上游 SHA + 重放本地改动；注：策略并未记载把补丁回合并上游，故"贡献"是理念层呼应，不等于已证的上游回馈）/`[inferred 映射]`。
