...
方案是把"派发语义"做成五个一等公民方法，每个方法内部封装好"要不要 await、要不要收返回值、能不能中途停下"这三件事的组合。五种模式一句话各记一个画面：`emit` 是广播喇叭（喊完就走），`parallel` 是群发抄送（人人一份、一起等），`serial` 是排队安检（一个个过、遇到"命中"就停），`bail` 是同步版的"问一圈取第一个有效答案"，`waterfall` 是接力赛（每棒能改交接物、也能不传棒直接冲线）。之所以是五个独立方法、而不是一个带 mode 参数的 `dispatch(mode, name, ...)`，第一性的理由是：派发语义本质是通信的**契约**而非运行时选项——把它编码进方法名与差异化的类型签名（`waterfall`/`parallel`/`emit` 的返回类型各异，`events.ts:19-28`），TypeScript 就能在编译期挡住"对 emit 事件取返回值"这类错误；仓库里那个已标 `@deprecated` 的单一入口 `dispatch(type, args)`（`events.ts:83-87`）反向印证了单一入口不是首选方向。[verified]
...
于是"注册一个 listener"天然是一次副作用：`hooks.push({ ctx, callback, ...options })` 是"做了什么"，返回的 `unregister`（`events.ts:136-142`，用 `findIndex` + `splice` 把这条 hook 摘掉）就是"怎么撤销"。**插件卸载时，它注册过的所有事件监听会被 Cordis 自动摘除，无需插件作者手写清理**——这正是第 05 章所说的"listener 作为 Cordis effect 天然获得 HMR（Hot Module Replacement，热模块替换——改了代码不重启进程、只把受影响的模块换掉）与卸载回滚"在事件层的落点。对使用者来说，这意味着热重载一个插件时不会残留幽灵监听器，也不会重复注册。
...
flowchart LR
  ...
  subgraph GONE["插件卸载 或 热重载"]
    D["Cordis 逆序调 disposer"] --> U["unregister findIndex splice<br/>events.ts:137 139"]
...
把 Cordis 的事件系统放到 Node 原生 `EventEmitter`（只有即发即弃的 `emit` + 手动 `on/off`）与 koa 式 `compose` 洋葱中间件（只对应 waterfall 一种）旁边看，会更清楚它为何要一次内建五模式。第一性的理由是：一个 harness 需要的通信谱系（通知 / 否决 / 改写 / 扇出）本就不止一种，且每一种都要吃到 fiber 的卸载回滚；用 `EventEmitter` 缺短路与自动卸载、用 `compose` 只覆盖 waterfall 一角，剩下的要么手写、要么再引一个库，反而更碎。代价是五模式加 `isBailed` 的心智成本偏高。Cordis 侧五模式与 effect 注册均在源码可证（`events.ts:96-134`，[verified]），而 `EventEmitter`/`compose` 的能力边界属通用工程常识（[inferred]）。

## 七、仍存在的问题与局限

- **`deprecated dispatch` 仍在**：`events.ts:83-87` 的 `dispatch(type, args)` 已标 `@deprecated`，但代码尚未移除，返回的是 `callback.bind(thisArg)` 数组——属历史包袱，读者容易误当作现役 API（Application Programming Interface，应用编程接口，即对外公开、承诺可调用的方法）。
...
1. **五种现役 dispatch 方法确实用 `Reflect.apply`**：`parallel`（`:91`）、`emit`（`:98`）、`serial`（`:104`）、`bail`（`:112`）、`waterfall`（`:122`）无一例外调 `Reflect.apply(callback, thisArg, args)`。就"派发时如何调用 callback"而言，`Reflect.apply` 确是现役机制。
