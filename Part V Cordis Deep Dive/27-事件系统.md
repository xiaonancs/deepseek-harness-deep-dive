...
于是"注册一个 listener"天然是一次副作用：`hooks.push({ ctx, callback, ...options })` 是"做了什么"，返回的 `unregister`（`events.ts:136-142`，用 `findIndex` + `splice` 把这条 hook 摘掉）就是"怎么撤销"。**插件卸载时，它注册过的所有事件监听会被 Cordis 自动摘除，无需插件作者手写清理**——这正是第 05 章所说的"listener 作为 Cordis effect 天然获得 HMR 与卸载回滚"在事件层的落点。对使用者来说，这意味着热重载一个插件时不会残留幽灵监听器，也不会重复注册。
