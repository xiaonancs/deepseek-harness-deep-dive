...
**门禁分层：本地薄、CI 厚。** Lefthook 的两个钩子是快速本地检查点，只拦「便宜、高把握」的缺陷；exhaustive（穷尽）覆盖归 CI `[verified]`（`fast-local-git-hooks.md:15-19`）。
...
- `package.json:33`（duplication/jscpd）、`:35`（test:coverage 覆盖率闸）、`:63`（knip）、`:107`（verify-cordis-api → gen-cordis-api.ts --check）、`:104-125`（gen/verify --check 生成式门禁族）、`:128`（hygiene `&&` 链）
