...
按主题分，实现态（`implemented/`）的笔记共 514 篇（截至 rc.7 快照；正文 rc.5 基线时约 505 篇，与第 20 章一致）[verified `repo/deepseek-harness/.agents/notes/implemented/`]：architecture 130、feature 172、bug-fix 82、process 69、simplification 48、testing 13。另有 `proposed/`（提案态）25 篇、`rejected/`（已拒）11 篇、`archived/`（归档）142 篇 [verified]。数量不是重点——重点是它们连缀出的那张「决策地图」。
